...
def dphidt(phi, p):
    #return -phi * np.minimum(max_decay, np.maximum(0, np.square(kappa(phi)) * dp2(p) - psi(phi)))
    #return -phi * np.minimum(max_decay, np.square(kappa(phi)) * np.maximum(0, dp2(p) - psi(phi)))
    return -phi * np.minimum(max_decay, np.maximum(0, dp2(p) - psi(phi)))
...
def update(frame):
    ...
    for itr in range(5):
        ...
        print(np.min(dphi), np.max(dphi), t)
